...
# за 14-20 июля 2026 (Минск, UTC+3), с обработкой ВСЕХ страниц пагинации.
...
# ── ИСПРАВЛЕНО: 14-20 июля 2026 (Минск, UTC+3), а не 2025 ──
...
echo "Диапазон: $(date -d @$TS_FROM) — $(date -d @$TS_TO)"
echo "Скачиваю страницы..."
while true; do
  resp="$TMPDIR/page_$page.json"
  code=$(curl -s -g -o "$resp" -w "%{http_code}" \
    "https://daangrah000.amocrm.ru/api/v4/leads?filter[statuses][0][pipeline_id]=8568342&filter[statuses][0][status_id]=142&filter[closed_at][from]=$TS_FROM&filter[closed_at][to]=$TS_TO&with=tags,custom_fields_values&limit=250&page=$page" \
